...
namespace FinalProject.Database
{
	class DbMySQL : DBConnection
	{
		...
		public Employee[] findSecutery()
		{
			Employee[] emp = null;
			DataSet ds = new DataSet();
			string cmdStr = "SELECT * FROM `employee` WHERE (`role`=\"מזכיר\")OR(`role`=\"מזכירה\")";
			using (MySqlCommand command = new MySqlCommand(cmdStr))
			{
				ds = GetMultipleQuery(command); // set dataset object with command query object 
			}
		...
		// Adds again Candidate Car to Database
		public void ChengeCandidateCar(Car Item)
		{
			string cmdStr = "UPDATE `candidate_car` SET `status`=@status WHERE `license_number`=@license_number";
			using (MySqlCommand command = new MySqlCommand(cmdStr))
			{
				command.Parameters.AddWithValue("@license_number", Item.LicenseNumber);
				command.Parameters.AddWithValue("@status", Item.Status);
		...
		public void deletMetting(Request requests)
		{
			string cmdStr = "DELETE FROM `requests` WHERE `NumberRequest`=@NumberRequest";
			using (MySqlCommand command = new MySqlCommand(cmdStr))
			{
				command.Parameters.AddWithValue("@NumberRequest", requests.NumberRequest);
				base.ExecuteSimpleQuery(command);
			}
		}

		// Adds Meeting info to Database
		public void InsertMeeting(string id, string description, DateTime date, string houers)
		{
			string cmdStr = "INSERT INTO requests (Id, DateToComunity, explanationOfRequest,requestHouers)" +
				"VALUES(@Id, @DateToComunity, @explanationOfRequest,@requestHouers)";
			using (MySqlCommand command = new MySqlCommand(cmdStr))
			{
				command.Parameters.AddWithValue("@Id", id);
				command.Parameters.AddWithValue("@DateToComunity", date);
				command.Parameters.AddWithValue("@explanationOfRequest", description);
				command.Parameters.AddWithValue("@requestHouers", houers);
				base.ExecuteSimpleQuery(command);
			}
		}
		...
		public Contract ContractToMissionListFind(int ContractNumber)
		{
			...
			if (dt.Rows.Count > 0)
			{
				...
				for (int i = 0; i < dt.Rows.Count; i++)
				{
					cos = new Costumer(dt.Rows[i][21].ToString(), dt.Rows[i][22].ToString(), dt.Rows[i][23].ToString(), dt.Rows[i][26].ToString(), dt.Rows[i][25].ToString(), dt.Rows[i][24].ToString());
					c = new Car(dt.Rows[i][4].ToString(), (int)dt.Rows[i][5], dt.Rows[i][6].ToString(), dt.Rows[i][7].ToString(), dt.Rows[i][8].ToString(), dt.Rows[i][9].ToString(), (int)dt.Rows[i][10], dt.Rows[i][11].ToString(), dt.Rows[i][12].ToString(), dt.Rows[i][13].ToString(), (DateTime)dt.Rows[i][14], dt.Rows[i][15].ToString(), dt.Rows[i][16].ToString(), dt.Rows[i][17].ToString(), (DateTime)dt.Rows[i][19], (double)dt.Rows[i][18]);
					contract= new Contract(c, cos, (int)dt.Rows[i][0], (DateTime)dt.Rows[i][3]);
				}
			}
			return contract;
		}
				...
				for (int i = 0; i < missions.Length; i++)
				{
					if (dt.Rows[i][3].ToString() != "")
					{
						if (dt.Rows[i][6].ToString() != "")
							...
							mish = new MissionList((int)dt.Rows[i][0], dt.Rows[i][1].ToString(), (DateTime)dt.Rows[i][2], (DateTime)dt.Rows[i][3], (int)dt.Rows[i][4], (int)dt.Rows[i][5], 0);
					}
					else
					{
						if (dt.Rows[i][6].ToString() != "")
							...
							mish = new MissionList((int)dt.Rows[i][0], dt.Rows[i][1].ToString(), (DateTime)dt.Rows[i][2], ((DateTime)dt.Rows[i][2]).AddDays(-1), (int)dt.Rows[i][4], (int)dt.Rows[i][5], 0);
					}
		...
		public Mission[] MissionFinish(string id)
		{
			...
			string cmdStr = "SELECT `mission`.*,`car`.`license_number` FROM `mission` JOIN `contract`,`car` WHERE (`mission`.`id` IS NOT NULL)AND(`mission`.`Houers`>0)AND(`mission`.`id`=" + id + ")AND(`contract`.`ContractNum`=`mission`.`ContractNumber`)AND(`contract`.`LicenseNumber`=`car`.`license_number`)AND(`mission`.`DateMission`>=\"" + dateTime.Date.ToString("yyyy-MM-dd") + "\")";
			using (MySqlCommand command = new MySqlCommand(cmdStr))
			{
				ds = GetMultipleQuery(command); // set dataset object with command query object 
			}
			...
			if (dt.Rows.Count > 0)
			{
				...
				for (int i = 0; i < myMission.Length; i++)
				{
					if (dt.Rows[i][0].ToString() != "")
						...
						mish = new Mission("", (int)dt.Rows[i][1], dt.Rows[i][2].ToString(), dt.Rows[i][3].ToString(), (int)dt.Rows[i][4], (DateTime)dt.Rows[i][5], (int)dt.Rows[i][6], (int)dt.Rows[i][8]);
					myMission[i] = mish;
				}
			}
			return myMission;
		...
		public void finishMissionWhisChengeCar(Mission miss, string workId, int houers, int chengeCar)
		{
			...
			string cmdStr = "UPDATE `mission` SET `id`=@id, `Mission`= @Mission,`DateMission`=@DateMission, `FaultDescription`=@FaultDescription,`Houers`=@Houers,license_number_of_Chenge_car=@license_number_of_Chenge_car" +
				" WHERE `mission`.`EventNumber`=" + miss.MissionID;
			using (MySqlCommand command = new MySqlCommand(cmdStr))
			{
				command.Parameters.AddWithValue("@Mission", miss.Status);
				...
				command.Parameters.AddWithValue("@DateMission", dateTime.Date.ToString("yyyy-MM-dd"));
				base.ExecuteSimpleQuery(command);
			}
		}
		...
		//add new Garage to garage to data base  
		public void insertGarage(Garage item)
		{
			string cmdStr = "INSERT INTO `garage`(`Id`, `NameOfGarage`, `PhoneNumber`, `street`, `city`, `NumberHouse`) VALUES (@Id,@NameOfGarage,@PhoneNumber,@street,@city,@NumberHouse)";
			using (MySqlCommand command = new MySqlCommand(cmdStr))
			{
				command.Parameters.AddWithValue("@Id", item.ID);
				command.Parameters.AddWithValue("@NameOfGarage", item.GarageName);
				command.Parameters.AddWithValue("@PhoneNumber", item.PhoneNumber);
				command.Parameters.AddWithValue("@street", item.Street);
				command.Parameters.AddWithValue("@city", item.City);
				command.Parameters.AddWithValue("@NumberHouse", item.HouseNumber);
		...
		//function to drop car for wating to chenge the car to work car 
		public void DropCar(int licenceNum)
		{
			string cmdStr = "DELETE FROM `car` WHERE`license_number`=@license_number";
			using (MySqlCommand command = new MySqlCommand(cmdStr))
			{
				command.Parameters.AddWithValue("@license_number", licenceNum);
				base.ExecuteSimpleQuery(command);
		...
		//function to find car betwen chenge purpoese of car from data base 
		public Car[] FindCarToChengePur(int licenceNum)
		{
			Car[] car = null;
			DataSet ds = new DataSet();
			string cmdStr = "SELECT * FROM `car` WHERE (car.license_number NOT IN(SELECT contract.LicenseNumber FROM contract WHERE 1))AND(car.license_number LIKE '%" + licenceNum + "%')";
			using (MySqlCommand command = new MySqlCommand(cmdStr))
			{
				ds = GetMultipleQuery(command); // set dataset object with command query object 
			}
		...
		public ExceptionHours[] ListOfWorkingHours(DateTime dateTime,int houers)
		{
			ExceptionHours[] exceptionHours = null;
			DataSet ds = new DataSet();
			DateTime limit = dateTime;
			limit = limit.AddMonths(1);
			string cmdStr = "SELECT `mission`.*,`employee`.* FROM `mission` JOIN `employee` WHERE (`Houers`>="+ houers + ")AND(`mission`.`id`=`employee`.`id`)AND(`mission`.`DateMission`>=\"" + dateTime.Date.ToString("yyyy-MM-dd") + "\")AND(`mission`.`DateMission`<\"" + limit.Date.ToString("yyyy-MM-dd") + "\")";
			using (MySqlCommand command = new MySqlCommand(cmdStr))
			{
				ds = GetMultipleQuery(command); // set dataset object with command query object 
			}
		...
		public Employee[] ListOfRequests(ref Request[] r)
		{
			...
			DataSet ds = new DataSet();
			string cmdStr = "SELECT `employee`.*,`requests`.* FROM `employee` JOIN `requests` WHERE (`employee`.id=`requests`.Id)AND(`DateToComunity`>" + date.Date.ToString("yyyy-MM-dd") + ")";
			using (MySqlCommand command = new MySqlCommand(cmdStr))
			{
				ds = GetMultipleQuery(command); // set dataset object with command query object 
			}
			...
			if (dt.Rows.Count > 0)
			{
				...
				for (int i = 0; i < dt.Rows.Count; i++)
				{
					...
					r[i] = new Request((int)dt.Rows[i][10], dt.Rows[i][11].ToString(), (DateTime)dt.Rows[i][12], dt.Rows[i][13].ToString(), dt.Rows[i][14].ToString());
				}
			}
			Disconnect();
			return emp;
		...
		public void InsertEmployeeLogin(string ID, string pass)
		{
		...
		public string[] findMail()
		{
			DataSet ds = new DataSet();
			string[] emailim = null;
			string cmdStr = "SELECT mail FROM `employee` WHERE role=\"טסטר\"OR role=\"מנהל משימות\"";
			using (MySqlCommand command = new MySqlCommand(cmdStr))
			{
				ds = GetMultipleQuery(command); // set dataset object with command query object 
			}
			...
			if (dt.Rows.Count > 0)
			{
				emailim = new string[dt.Rows.Count];
				for (int i = 0; i < dt.Rows.Count; i++)
					emailim[i] = dt.Rows[i][0].ToString();
			}
			Disconnect();
			return emailim;
		}
		public string[] findMailSecutery()
		{
			DataSet ds = new DataSet();
			string[] emailim = null;
			string cmdStr = "SELECT mail FROM `employee` WHERE role=\"מזכירה\"";
			using (MySqlCommand command = new MySqlCommand(cmdStr))
			{
				ds = GetMultipleQuery(command); // set dataset object with command query object 
			}
		...
		public string SecuteryMainChouseQueryParking(string chouse, string info)
		{
			...
			else if (ch.Equals("יצרן/דגם"))
			{
				...
				if (str.Length == 2 && str[0] != "" && str[1] != "")
					...
					query += "AND((`car`.`visible_brand` LIKE\"%" + str[0] + "%\")OR(car.visible_model LIKE \"%" + str[0] + "%\"))";
			}
				...
				switch (chouse)
				{
					...
					case 1:
						...
						if (split.Length == 2 && split[0] != "" && split[1] != "")
							...
							query += "AND((`car`.`visible_brand`='" + split[0] + "')OR(`car`.`visible_model`='" + split[0] + "'))";
						else
							return "נתון שגוי";
						break;
		...
		public string bildQueryForCarsHistory(string name)
		{
			if (name.Equals("missions"))
				return "SELECT `mission`.*,`car`.`license_number` FROM `mission` JOIN `contract`,`car`,`missionlist`,`customers` WHERE(`contract`.`ContractNum`=`mission`.`ContractNumber`) AND(`contract`.`LicenseNumber`=`car`.`license_number`) AND(`missionlist`.`EventNumber`=`mission`.`EventNumber`) AND(`contract`.`CostumerID`=`customers`.`ID`)";
			if (name.Equals("missionLists"))
				return "SELECT * FROM `missionlist` JOIN `customers`,`contract`,`mission`,`car` WHERE(`contract`.`CostumerID`=`customers`.`ID`)AND(`car`.`license_number`=`contract`.`LicenseNumber`)AND(`missionlist`.`EventNumber`=`mission`.`EventNumber`)AND(`customers`.`ID` IN(SELECT `contract`.`CostumerID` FROM `mission` JOIN `missionlist`,`contract` WHERE `missionlist`.`EventNumber`=`mission`.`EventNumber`)AND(`contract`.`ContractNum`= mission.ContractNumber)";
			return "";
		}
		...
		// Utility function for Search in monthly exception hours
		public string bildQueryToFindEmployee(string names, string info,int houers)
		{
			string[] splitNames, splitInfo, spliter;
			string query = "SELECT `mission`.*,`employee`.* FROM `mission` JOIN `employee` WHERE (`Houers`>="+ houers + ")AND(`mission`.`id`=`employee`.`id`) ";
			spliter = names.Split(':');
			splitNames = new string[spliter.Length - 1];
			for (int i = 0, j = 0; i < spliter.Length; i++)
				if (spliter[i] != "")
			...
			for (int i = 0; i < splitNames.Length; i++)
			{
				...
				if (splitNames[i].Equals("comboMonths"))
					query += "AND(`mission`.`DateMission`>=\"" + splitInfo[i] + "\")AND(`mission`.`DateMission`<=\"" + splitInfo[i+1] + "\")";
			}
			return query;
		}
		...
		{
			string[] splitNames, splitInfo, spliter, spl;
			spliter = names.Split(':');
			string query = "";
			splitNames = new string[spliter.Length - 1];
			for (int i = 0, j = 0; i < spliter.Length; i++)
			...
			for (int i = 0; i < splitNames.Length; i++)
			{
				...
				if (splitNames[i].Equals("textBrandAndModel"))
				{
					spl = splitInfo[i].Split('/');
					if (spl.Length == 2 && spl[0] != string.Empty && spl[1] != string.Empty)
						query += "AND(`visible_brand` LIKE '%" + spl[0] + "%')AND(`visible_model`LIKE '%" + spl[1] + "%')";
					else if (spl[0] != string.Empty)
						query += "AND((`visible_brand` LIKE '%" + spl[0] + "%')OR(`visible_model`LIKE '%" + spl[0] + "%'))";
				}
				if (splitNames[i].Equals("textName"))
				{
					spl = splitInfo[i].Split(' ');
					if (spl.Length == 2 && spl[0] != string.Empty && spl[1] != string.Empty)
						query += "AND(`customers`.`FirstName` LIKE '%" + spl[0] + "%')AND(`customers`.`LastName` LIKE '%" + spl[1] + "%')";
					else if (spl[0] != string.Empty)
						query += "AND((`customers`.`FirstName` LIKE '%" + spl[0] + "%')OR(`customers`.`LastName` LIKE '%" + spl[0] + "%'))";
				}
